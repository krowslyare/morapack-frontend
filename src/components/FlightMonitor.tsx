...
interface FlightMonitorProps {
  simulationResults?: unknown // legacy prop for compatibility
  simulationType?: SimulationType
  activeFlights?: ActiveFlight[]
  currentSimDateTime?: Date
}
...
function AnimatedFlights({
  ...
  const lastStateRef = useRef<Record<string, MarkerSnapshot>>({})
  ...
  useEffect(() => {
    ...
    flights.forEach((f) => {
      ...
      const dx = dlng * Math.cos(latRad)
      const bearing = Math.atan2(dlat, dx)
      const rotation = bearing - Math.PI // sprite apunta a la izquierda por defecto

      let marker = markersRef.current[markerId]
      if (!marker) {
        const planeHTML = `<img src="/airplane.png" alt="✈" class="plane"
          style="width:20px;height:20px;display:block;transform-origin:50% 50%;will-change:transform;transition:transform 0.25s linear;" />`
      ...
      if (plane) {
        plane.style.transform = `rotate(${rotation}rad)`
      }

      lastStateRef.current[markerId] = { progress: f.progress, lat, lng, angle: rotation }
    })
...
export function FlightMonitor(props: FlightMonitorProps) {
  const {
  ...
  const [selectedFlight, setSelectedFlight] = useState<SimFlight | ActiveFlight | null>(null)
  const [selectedAirport, setSelectedAirport] = useState<SimAirport | null>(null)

  // Reloj simple para animación demo
  ...
  const temporalFlights: SimulatedActiveFlight[] = useMemo(() => {
    if (!Array.isArray(activeFlights)) return []

    return activeFlights.map((flight) => ({
      flightId: flight.flightId,
      flightCode: flight.flightCode || `FLT-${flight.flightId}`,
  ...
  const demoFlights: SimulatedActiveFlight[] = useMemo(() => {
  ...
  const isTemporalMode = simulationType === 'day-to-day' && Array.isArray(activeFlights)
  const flightsToRender = isTemporalMode ? temporalFlights : demoFlights
  ...
  return (
    <MonitorWrapper>
      ...
      <MapWrapper>
        {simulationType === 'day-to-day' && currentSimDateTime && (
          <RealtimeOverlay>
            <OverlayLabel>Tiempo simulado</OverlayLabel>
            <OverlayValue>
            ...
            <OverlayHint>
              {flightsToRender.length} vuelo{flightsToRender.length === 1 ? '' : 's'} en ruta
            </OverlayHint>
          </RealtimeOverlay>
        )}
        <MapContainer
        ...
        >
          ...
          {flightsToRender.length > 0 && (
            <>
              <RoutesLayer flights={flightsToRender} airports={airports} />
              <AnimatedFlights
          ...
          <Legend>
            ...
            <div style={{ fontWeight: 600 }}>
              {simulationType === 'day-to-day'
                ? 'Aviones activos (timeline temporal)'
                : 'Aviones en movimiento (simulación semanal)'}
            </div>
